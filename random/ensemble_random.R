...
create_uncertainty_ensemble <- function(data, target_col, n_models = 20) {
  models <- list()
  ...
  for(i in 1:n_models) {
    ...
    model <- randomForest(as.formula(formula_str), data = boot_data, 
                          ntree = 5,        # pochissimi alberi
                          mtry = 1,         # solo 1 feature 
                          nodesize = 50,    # nodi molto grandi
                          maxnodes = 3)     # alberi piccolissimi
    models[[i]] <- model
  }
  return(models)
}
